...
\title{NewOz: Steps toward a modern syntax for the Oz programming language}
\subject{Master thesis under the supervision of Prof. Peter Van Roy}
\author{Martin Vandenbussche - 02441500}
\date{Academic year 2020--2021}

\begin{document}

    % Handled by the EPL format (see Makefile)
%\maketitle

\input{abstract.tex}

\setcounter{tocdepth}{1} % Show up to sections in TOC
\tableofcontents
...
~\nocite{mozart2notation,ThesisGitHub,wikiScala,van2020history,Kornstaedt96}
\printbibliography[keyword=ref]
\printbibliography[keyword=software,title={Software and Tools}]
\printbibliography[keyword=perso,title={Personal contributions}]
...
It has also influenced the development of other languages such as \textit{Scala} and Go.
...
The goal is to make a completely new syntax of Oz, taking advantage of the best parts of \textit{Scala}, Python, Go, Clojure, and other modern languages.
